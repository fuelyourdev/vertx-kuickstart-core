...
/**
 * Helper function for combining an object and a patch, such that patch fields
 * override the fields in the object if they are present, otherwise the fields
 * from the object come through. Note that the object and patch remain
 * unchanged,and a new object is created and returned.
 */
...
fun <T : Any> T.applyPatch(patch: Any): T {
    if (this::class.isData) {
        ...
        val args = patch::class.declaredMemberProperties
            ...
            .filter {
                with(it.second) {
                    (this is Field<*> && present) ||
                        (this !is Field<*> && this != null)
                }
            }
            .map {
                it.first to with(it.second) {
