...
class DeserializerInstantiateTest : StringSpec() {
    ...
    init {
        ...
        ("A Field param distinguishes between missing params and " +
            "explicit nulls") {
        ...
        ("instantiate throws an exception if parameter is null and is " +
            "not nullable") {
        ...
        ("instantiate for enum with a value not in the enum throws " +
            "an exception") {
            ...
            exception.message shouldBe
                "Enum MyEnum does not contain value: NotInEnum"
        }
        ...
        "instantiate for enum with a non string value throws an exception" {
            ...
            exception.message shouldBe
                "ClassWithEnum.myEnum expects type MyEnum " +
                "but was given the value: 1"
            exception.shouldHaveCauseInstanceOf<ClassCastException>()
        }

        ("instantiate for custom class with list generics shouldn't " +
            "loose generics") {
        ...
        "An array of Field type throws an exception" {
            data class FieldArray(
                ...
                override fun equals(other: Any?): Boolean {
                    ...
                    if (!fieldArray.contentEquals(other.fieldArray))
                        return false
        ...
        "instantiate works with 2d arrays" {
            ...
            val json = jsonObjectOf(
                "int2DArray" to jsonArrayOf(
                    jsonArrayOf(1, 2),
                    jsonArrayOf(3, 4)
                ),
                "intObj2DArray" to jsonArrayOf(
                    jsonArrayOf(5, 6),
                    jsonArrayOf(7, 8)
                )
            )
        ...
        "instantiate works for a data class with an array" {
            data class ClassWithArray(
                ...
                override fun equals(other: Any?): Boolean {
                    ...
                    if (!booleanArr.contentEquals(other.booleanArr))
                        return false
                    if (!booleanObjArr.contentEquals(other.booleanObjArr))
                        return false
                    if (!doubleArr.contentEquals(other.doubleArr))
                        return false
                    if (!doubleObjArr.contentEquals(other.doubleObjArr))
                        return false
                    if (!floatArr.contentEquals(other.floatArr))
                        return false
                    if (!floatObjArr.contentEquals(other.floatObjArr))
                        return false
                    if (!intArr.contentEquals(other.intArr))
                        return false
                    if (!intObjArr.contentEquals(other.intObjArr))
                        return false
                    if (!longArr.contentEquals(other.longArr))
                        return false
                    if (!longObjArr.contentEquals(other.longObjArr))
                        return false
                    if (!stringArr.contentEquals(other.stringArr))
                        return false
                    if (!instantArr.contentEquals(other.instantArr))
                        return false
                    if (!listArr.contentEquals(other.listArr))
                        return false
                    if (!mapArr.contentEquals(other.mapArr))
                        return false
